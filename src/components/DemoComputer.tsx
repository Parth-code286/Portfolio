...
function DemoComputer({ scale = 1, position = [0, 0, 0], rotation = [0, 0, 0] }: DemoComputerProps) {
  const group = useRef<THREE.Group>(null)
  const { nodes, materials } = useGLTF('/models/new.glb') as any
  ...
  useEffect(() => {
    ...
    // Type guard for checking if object is a mesh with material
    ...
    // Traverse all objects and set material properties for better sharpness
  ...
  useFrame(({ clock }: { clock: THREE.Clock }) => {
    if (group.current) {
      group.current.rotation.y = Math.sin(clock.getElapsedTime() * 0.5) * 0.1
    }
  })
  return (
    <group 
